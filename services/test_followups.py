...
def test_send_due_followups_skips_not_due_and_already_sent(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        _async(lambda **kw: sent_to.append(kw["phone"])),
    )
    monkeypatch.setattr(
    ...
    sent = _run(send_due_followups(visits, now=datetime(2026, 1, 3)))
...
def test_one_failed_task_creation_does_not_abort_the_batch(monkeypatch):
    ...
    async def flaky_create_task(**kw):
        if kw["phone"] == "+2222":
    ...
    sent = _run(send_due_followups(visits, now=datetime(2026, 1, 3)))  # must not raise
